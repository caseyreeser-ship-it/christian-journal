...
export default function DailyJournal({ entry, onChange }) {
  ...
  return (
    <section className="journal-stack">
      <JournalPrompt label="How did God show up yesterday?" value={entry.godShowedUp} onChange={(v) => setField('godShowedUp', v)} />
      <JournalPrompt label="How did I feel encouraged?" value={entry.encouraged} onChange={(v) => setField('encouraged', v)} />
      <JournalPrompt label="What can I work on / need prayer for?" value={entry.prayer} onChange={(v) => setField('prayer', v)} />
